...
@Component({
  selector: 'facehiring-company-profile',
...
export class CompanyProfileComponent implements OnInit {
  ...
  public getEmployeeProfile(employeeID)
    ...
    async loadCompanyContent(employeeID) {
      ...
      let res = await Promise.all([similarProfiles, updates, careers,activeJobs]);
      this.similarCompanyProfiles = res[0];
      this.companyUpdates = res[1];
      this.companyCareers = res[2];
      this.activeJobPostings = res[3];
